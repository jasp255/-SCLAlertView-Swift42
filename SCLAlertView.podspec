Pod::Spec.new do |s|
  s.name         = "SCLAlertView"
  s.version      = "0.8.1"
  s.summary      = "Beautiful Alert View. Written in Swift, only for Swift 4.2"
  s.homepage     = "https://github.com/vikmeup/SCLAlertView-Swift"
  s.screenshots  = "https://raw.githubusercontent.com/vikmeup/SCPopUpView/master/errorScreenshot.png", "https://raw.githubusercontent.com/vikmeup/SCPopUpView/master/successScreenshot.png"
  s.license      = { :type => "MIT", :file => "LICENCE" }
  s.author             = { "Jose Antonio" => "jasp255@gmail.com" }
  s.social_media_url   = "http://twitter.com/jasp255"
  s.platform     = :ios
  s.ios.deployment_target = '8.0'
  s.source       = { :git => "https://github.com/vikmeup/SCLAlertView-Swift42.git", :tag => s.version }
  s.source_files  = "SCLAlertView/*.{swift}"
  s.swift_version = '4.2'
  #s.framework  = "QuartzCore"
  s.requires_arc = true
end
